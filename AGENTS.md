...
- Workspace MSRV is Rust 1.87, verified by a dedicated `msrv` CI job (it was declared as 1.74 for
  months while the code required 1.87, because every job used `stable`). CI also runs
  `cargo fmt --check`, clippy, tests, and full macOS release-style builds.
