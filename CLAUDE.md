...
- Presence images are resolved by `ImagesConfig::resolve`, not passed through raw: `mode = "url"`
  ...
  symptom is a blank tile with nothing in the logs.
- The art is each app's **own logo** (vendored glyph data in `assets/discord/sources.json` from SVG
  Logos / Devicon / vscode-icons / Simple Icons — CC0 or MIT, all four), unified by one tile
  treatment rather than by redrawing anything. Only eight apps with no logo in any of those sets get a
  monogram. `assets/discord/overrides/{agents,hosts}/<name>.{png,svg}` replaces a generated tile
  verbatim — the seam for a third-party pack, with the licence caveat documented, since anything there
  is redistributed. Do **not** vendor art from Behance/DeviantArt-style packs without an explicit
  redistribution grant. **`mode = "url"` is likewise unverified against a
