...
- **Presence art for every agent and host app** — 45 512×512 PNGs in `assets/discord/`, generated by
  `scripts/build-discord-assets.py`. Each app wears its own real multi-colour logo (VS Code's ribbon,
  the JetBrains SKU tiles, Xcode's hammer, Ghostty's ghost), unified by one treatment: a tile tinted
  toward that logo's dominant colour, with a light tile for marks that are essentially black so they
  do not vanish. Glyph data is vendored in `assets/discord/sources.json` from four sets distributed on
  npm — SVG Logos (CC0), Devicon (MIT), vscode-icons (MIT), Simple Icons (CC0) — so a rebuild needs no
  network. The eight apps with no logo in any of them (Kitty, Tabby, Nova, RustRover, Amp, Aider,
  Crush, Goose) get a monogram in the same tile rather than a traced look-alike.
- **`assets/discord/overrides/`**: drop in `<name>.png` or `<name>.svg` and it replaces the generated
  tile verbatim, so a third-party icon pack needs no code change. The build script reports which files
  came from there; the README spells out the licence question, since an override is redistribution.
